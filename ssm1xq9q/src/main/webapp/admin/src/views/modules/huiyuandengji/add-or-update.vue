<template>
  <div class="addEdit-block">
    ...
    >
      <el-row >
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="会员等级" prop="huiyuandengji">
          <el-input v-model="ruleForm.huiyuandengji" 
              placeholder="会员等级" clearable  :readonly="ro.huiyuandengji"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="会员等级" prop="huiyuandengji">
              <el-input v-model="ruleForm.huiyuandengji" 
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `huiyuandengji/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.huiyuandengjiCrossAddOrUpdateFlag = false;
						 this.parent.search();
