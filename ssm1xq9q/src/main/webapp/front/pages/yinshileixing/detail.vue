...
<script>
	export default {
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','yinshileixing');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				uni.setStorageSync('crossTable','yinshileixing');
			...
			// 获取详情
			async init(){
				let res = await this.$api.info('yinshileixing', this.id);
			...
			// 添加评论
			async onCommentTap() {
				this.$utils.jump(`../discussyinshileixing/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
			...
			async onFinishTap() {
				...
				await this.$api.update('yinshileixing', this.detail);
				this.$utils.msg('审核成功');
