...
<script>
	export default {
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','huiyuandengji');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				uni.setStorageSync('crossTable','huiyuandengji');
			...
			// 获取详情
			async init(){
				let res = await this.$api.info('huiyuandengji', this.id);
			...
			// 添加评论
			async onCommentTap() {
				this.$utils.jump(`../discusshuiyuandengji/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
			...
			async onFinishTap() {
				...
				await this.$api.update('huiyuandengji', this.detail);
				this.$utils.msg('审核成功');
