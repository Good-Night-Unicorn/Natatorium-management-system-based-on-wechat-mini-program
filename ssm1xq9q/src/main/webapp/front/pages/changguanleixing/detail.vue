...
<script>
	export default {
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','changguanleixing');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				uni.setStorageSync('crossTable','changguanleixing');
			...
			// 获取详情
			async init(){
				let res = await this.$api.info('changguanleixing', this.id);
			...
			// 添加评论
			async onCommentTap() {
				this.$utils.jump(`../discusschangguanleixing/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
			...
			async onFinishTap() {
				...
				await this.$api.update('changguanleixing', this.detail);
				this.$utils.msg('审核成功');
