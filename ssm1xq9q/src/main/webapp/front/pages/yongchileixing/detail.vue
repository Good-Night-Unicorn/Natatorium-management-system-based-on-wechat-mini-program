...
<script>
	export default {
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','yongchileixing');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				uni.setStorageSync('crossTable','yongchileixing');
			...
			// 获取详情
			async init(){
				let res = await this.$api.info('yongchileixing', this.id);
			...
			// 添加评论
			async onCommentTap() {
				this.$utils.jump(`../discussyongchileixing/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
			...
			async onFinishTap() {
				...
				await this.$api.update('yongchileixing', this.detail);
				this.$utils.msg('审核成功');
